...
export default async function Page() {
  ...
  const roadblockRes = await fetch(`${backend_uri}/api/roadblocks/network/jhorahat.com`, options)
  ...
  const [heroData, categoryBlocks] = await Promise.all([
    fetch(`${backend_uri}/api/posts/featured-news/jhorahat.com`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => []),

    fetch(`${backend_uri}/api/posts/homepage/jhorahat.com`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => [])
  ])
